...
class DownloadConfiguration extends React.Component {
    ...
    componentDidMount() {
        ...
        this.basemapLayer = L.tileLayer(basemap.url, basemap.options).addTo(this.map)
    ...
    componentDidUpdate(prevProps) {
        ...
        if (center[0] !== prevProps.center[0] || center[1] !== prevProps.center[1]) {
    ...
    render() {
        return <div className="preview-map-container">
            <div ref={input => this.mapNode = input} className="preview-map"></div>
        </div>
    }
}
