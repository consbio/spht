...
class DownloadStep extends Component {
    ...
    render() {
        let { map, configuration, advanced, isFetching, errorModal, onSubmit } = this.props
        ...
        return (
            <div className="configuration-step">
                ...
                >
                    { isFetching ? <div className="pageloader is-active"><span className="title">Creating report...</span></div> : null }
                    {
...
const mapStateToProps = ({ map, configuration, advanced, report, error }) => {
    return {map, configuration, advanced, isFetching: report.reportIsFetching, errorModal: error.showModal}
}
...
const mapDispatchToProps = dispatch => bindActionCreators({
    onSubmit: mapConfig => (_, getState) => {
        ...
        dispatch(createReport(
            bounds, zoom, basemap.url, colors[colorScheme], species, distribution,
            Object.keys(model).filter(item => model[item]), layerOpacity
        ))
    }
}, dispatch)
